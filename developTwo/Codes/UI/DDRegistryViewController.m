...
@interface DDRegistryViewController()<ASIHTTPRequestDelegate, UITextFieldDelegate> {
	...
	ASIHTTPRequest* _registryRequest;
	ACMessageDialog* _registryDialog;
}
...
@implementation DDRegistryViewController
...
- (void)queryForVerification {
	...
	
	{
		...
		NSDictionary* requestParameters = @{ @"phone": mobile, @"use_type": @1 };
...
//	}
	...
	{
		NSURL* requestUrl = getRegistryUrl();
		NSLog(@"URL - %@", requestUrl);
	...
	
	{
		...
		[dialog setDismissHandler: ^(ACMessageDialog* dialog) {
			if(dialog == _registryDialog) {
				_registryDialog = nil;
				
				[_registryRequest clearDelegatesAndCancel];
				_registryRequest = nil;
			}
		}];
		[dialog show];
		
		_registryDialog = dialog;
